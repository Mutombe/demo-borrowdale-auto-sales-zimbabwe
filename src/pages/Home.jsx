    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-black">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]} alt="Borrowdale workshop" className="absolute inset-0 w-full h-[130%] object-cover object-center" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 2.5, ease: 'easeInOut' }} loading="eager" />
        </AnimatePresence>
      ...
      <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-orange-500/0 via-red-600 to-orange-500/0 z-20" />

      {/* Slide indicators -- bottom horizontal */}
      <div className="absolute bottom-24 sm:bottom-28 left-1/2 -translate-x-1/2 z-30 flex gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)} className={`h-[3px] transition-all duration-700 ${i === currentSlide ? 'w-12 bg-red-600' : 'w-4 bg-white/15 hover:bg-white/30'}`} aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity: textOpacity }}>
        {/* Spark icon */}
        <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="w-14 h-14 bg-red-600/10 border border-red-600/30 flex items-center justify-center mb-8">
          <Wrench size={24} weight="fill" className="text-red-600" />
        </motion.div>

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-red-500 text-xs sm:text-sm font-semibold uppercase tracking-[0.35em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero?.badge || 'Premium Selection'}
        </motion.p>

        <div className="overflow-hidden">
          {['BORROWDALE', 'CLASS', 'MOTORS.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '120%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.12, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className={`font-heading leading-[0.85] tracking-tighter ${line === 'CLASS' ? 'bg-gradient-to-r from-orange-400 via-red-600 to-red-500 bg-clip-text text-transparent' : 'text-white'}`} style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'CLASS' ? 900 : 200 }}>
                {line}
              </h1>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.2 }} className="flex flex-wrap items-center gap-4 mt-8">
          <div className="flex items-center gap-2 bg-red-600/10 border border-red-600/25 px-4 py-2.5">
            <ShieldCheck size={18} weight="fill" className="text-red-600" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-8">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-red-600 text-black px-8 py-4 text-sm uppercase tracking-[0.15em] font-bold transition-all duration-500 hover:bg-red-500 hover:shadow-xl hover:shadow-red-600/25" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero?.ctaPrimary || 'Browse Stock'} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <a href="tel:+263734053347" className="group inline-flex items-center gap-3 border border-white/15 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-red-600/40 hover:text-red-500 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }}><ArrowDown size={14} className="text-red-600/40" /></motion.div>
      </motion.div>
    </section>
  );
}
...
function MarqueeTicker() {
  ...
  return (
    <section className="bg-black border-y border-red-600/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-red-600/70 font-heading text-lg sm:text-2xl tracking-wider font-bold uppercase">{item}</span>
            <span className="text-red-600/15 text-xs">///</span>
          </span>
        ))}
      </div>
    </section>
    ...
    <section ref={ref} className="bg-neutral-900 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <p className="text-red-600/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Results</p>
          <h2 className="font-heading text-white leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            The Borrowdale <span className="text-red-600">Difference</span>
          </h2>
        </motion.div>
          ...
          {cases.map((c, i) => (
            <motion.div key={c.title} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="group">
              ...
              <div className="bg-black p-5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5 bg-red-600/10 border border-red-600/20 px-3 py-1.5">
                  <Clock size={12} className="text-red-600" />
                  <span className="text-red-500 text-[10px] uppercase tracking-wider font-bold">{c.days}</span>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-red-600/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Capabilities</p>
              <h2 className="font-heading text-white leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>Our <span className="text-red-600">Services</span></h2>
            </div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-red-600 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }} className={i === 0 ? 'sm:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2.2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-orange-500 to-red-500 z-10" />
                </Link>
              </motion.div>
            );
          })}
...
function StatsBand() {
  ...
  return (
    <section ref={ref} className="relative bg-black overflow-hidden">
      <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-red-600/40 to-transparent" />
      <div className="absolute bottom-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-red-600/40 to-transparent" />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats?.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="text-center relative">
              <div className="font-heading text-red-600 leading-none font-bold" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)' }}>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-red-600 mb-6" />
            <p className="text-red-600/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Est. Harare</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Borrowdale <span className="text-red-600">Story</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              With 181 five-star reviews and counting, Borrowdale Panelbeaters has built a reputation as Harare's most trusted auto body repair shop. Located on Nuffield Road, our state-of-the-art facility handles everything from minor cosmetic touch-ups to major structural repairs.
            </p>
            ...
            <div className="flex gap-10 sm:gap-16">
              <div><div className="text-red-600 font-heading text-3xl sm:text-4xl font-bold leading-none">1</div><div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Reviews</div></div>
              <div><div className="text-red-600 font-heading text-3xl sm:text-4xl font-bold leading-none">5.0</div><div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Rating</div></div>
              <div><div className="text-red-600 font-heading text-3xl sm:text-4xl font-bold leading-none">24hr</div><div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Tow Service</div></div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="overflow-hidden"><img src="https://images.unsplash.com/photo-1549317661-bd32c8ce0571?w=800&q=80" alt="Borrowdale workshop" className="w-full aspect-[4/5] object-cover object-center" loading="lazy" /></div>
            ...
            <div className="absolute -top-4 -right-4 sm:-right-6 bg-red-600 text-black p-5 sm:p-7 shadow-2xl">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-red-600 mb-6" />
            <p className="text-red-600/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Why Borrowdale</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Built on <span className="text-red-600">Trust</span>
            </h2>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex gap-0.5 mt-1">{[...Array(t.rating || 5)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-red-600" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-600 hover:border-red-600/30 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">{testimonials.map((_, i) => <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-red-600' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i+1}`} />)}</div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-600 hover:border-red-600/30 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
    ...
    <section ref={ref} className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative z-10 max-w-3xl mx-auto px-5 sm:px-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/contact" className="group inline-flex items-center justify-center gap-3 bg-red-600 text-black px-8 py-4 text-sm uppercase tracking-[0.15em] font-bold hover:bg-red-500 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            Visit Showroom <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <a href="https://wa.me/263734053347" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 border border-green-500/40 text-green-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-green-500/10 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
